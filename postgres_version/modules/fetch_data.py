...
def fetch_incremental_data(total_cnt):
    ...
    if r:
        ...
        match = re.search(pattern, current_hwm)
        current_hwm = int(match.group(1) + match.group(2) + match.group(3))
        print(f"Current HWM: {current_hwm}")
    else:
    ...
    for start_idx in range(start, end + 1, step):
        ...
        try:
            ...
            if max_create_date < current_hwm:
                print(f"Stop fetching data from {start_idx} to {end_idx}")
                print(f"because of standard_create_date more than {max_create_date}")
                break

            dfs_to_download.append(parsed_df)
        except requests.exceptions.RequestException as e:
            # error
            print(f"Error fetching data from {start_idx} to {end_idx}: {e}")
            raise e
    ...
    if dfs_to_download:
        ...
        final_df = pd.concat(dfs_to_download, ignore_index=True)
        ...
        if 'CRTR_DD' in final_df.columns:
            ...
            print(f"Filtering with max_create_date: {current_hwm}")
            
            # CRTR_DD가 필터 날짜보다 작은 row들 제거
            before_filter_count = len(final_df)
            final_df = final_df[final_df['CRTR_DD'].astype(int) >= current_hwm]
            after_filter_count = len(final_df)

            print(f"Filtered out {before_filter_count - after_filter_count} rows with CRTR_DD < {current_hwm}")
